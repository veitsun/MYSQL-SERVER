...
class Xcl_xpriority_list_tests : public Test {
 public:
  ...
  struct Test_list_deleter {
    void operator()(void *ptr) const {
      delete static_cast<Test_list *>(ptr);
    }
  };

  Test_list *sut() { return static_cast<Test_list *>(m_sut.get()); }
  const Test_list *sut() const {
    return static_cast<const Test_list *>(m_sut.get());
  }

 public:
  void push(const Direction &d) {
    if (d.m_is_front) {
      sut()->push_front(d.m_element);
    } else {
      sut()->push_back(d.m_element);
    }
  }
  ...
  // priority list
  std::unique_ptr<void, Test_list_deleter> m_sut{new Test_list(),
                                                 Test_list_deleter{}};
};
